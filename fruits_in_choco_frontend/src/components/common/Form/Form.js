...
export const Form = (props) => {
    return <form onSubmit={props.handleSubmit} className={style.form}>
        {
            props.fields.map(i => <div className={style.fieldWrapper}>
                <div className={style.label}>{i.label}</div>
                <Field className={style.field} placeholder={i.placeholder} name={i.name} component={i.component}
        ...
        {
            props.error && <div className={formsControlsStyle.formSummaryError}>
                {props.error}
            </div>
        }
        <div>
            <Button type="submit" disabled={props.isFetching}>{props.submitName}</Button>
        </div>
    </form>
};